...
import { useRouter as useSearchRouter } from './search-components/useRouter';
...
export const Default = (props: SearchExperienceProps) => {
  ...
  const qFromRoute =
    typeof nextRouter.query.q === 'string' ? nextRouter.query.q : '';

  const { isEditing, isPreview } = page.mode;
  const [pageNumber, setPageNumber] = useState(1);
  const [inputValue, setInputValue] = useState<string>(qFromRoute);
  ...
  const { setRouterQuery } = useSearchRouter();
